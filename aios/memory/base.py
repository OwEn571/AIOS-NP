...
class BaseMemoryManager:
    ...
    def _analyze_query_to_memory(self, query: MemoryQuery) -> 'MemoryNote':
        ...
        if "context" in metadata and "context" not in filtered_data:
            filtered_data["context"] = metadata.get("context", "General")
        if "timestamp" in metadata and "timestamp" not in filtered_data:
            filtered_data["timestamp"] = metadata.get("timestamp")
    ...
    def address_request(self, memory_syscall):
        ...
        if memory_syscall.query.operation_type in {"add_memory", "add_agentic_memory"}:
            memory_note = self._analyze_query_to_memory(memory_syscall.query)
    ...
    def retrieve_memory(self, memory_query: MemoryQuery):
        ...
        doc_ids = chroma_results.get('ids', [])
        metadatas = chroma_results.get('metadatas', [])
        distances = chroma_results.get('distances', [])
        doc_id_list = doc_ids[0] if doc_ids and isinstance(doc_ids[0], list) else doc_ids
        metadata_list = metadatas[0] if metadatas and isinstance(metadatas[0], list) else metadatas
        distance_list = distances[0] if distances and isinstance(distances[0], list) else distances

        retrieved_results = []
        for index, retrieve_memory in enumerate(retrieved_memories[:k]):
            memory_id = doc_id_list[index] if index < len(doc_id_list) else None
            ...
            retrieved_results.append({
                ...
                'context': retrieve_memory.context,
