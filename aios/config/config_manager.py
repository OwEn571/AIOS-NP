...
class ConfigManager:
    ...
    def _apply_environment_overrides(self):
        """Apply optional runtime overrides from environment variables."""
        llms_config = self.config.setdefault("llms", {})
        models = llms_config.setdefault("models", [])
        if not models:
            models.append({})

        primary_model = dict(models[0])

        llm_model = os.environ.get("AIOS_LLM_MODEL")
        llm_backend = os.environ.get("AIOS_LLM_BACKEND")
        llm_base_url = os.environ.get("AIOS_LLM_BASE_URL")
        llm_api_key = os.environ.get("AIOS_LLM_API_KEY")

        if llm_model:
            primary_model["name"] = llm_model
        if llm_backend:
            primary_model["backend"] = llm_backend
        if llm_base_url:
            primary_model["hostname"] = llm_base_url
        if llm_api_key:
            primary_model["api_key"] = llm_api_key

        models[0] = primary_model

        server_config = self.config.setdefault("server", {})
        server_host = os.environ.get("AIOS_SERVER_HOST")
        server_port = os.environ.get("AIOS_SERVER_PORT")
        if server_host:
    ...
    def get_mcp_server_script_path(self) -> str:
        ...
        """
        script_path = Path(self.config.get("tool", {}).get("mcp_server_script_path"))
        if script_path.is_absolute():
            return str(script_path)

        project_root = Path(__file__).resolve().parents[2]
        return str((project_root / script_path).resolve())
