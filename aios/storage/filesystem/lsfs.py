...
class LSFS:
    ...
    def address_request(self, agent_request):
        ...
        elif operation_type in ["create_dir", "delete_dir", "list_dir"]:
            path = agent_request.query.params.get("dir_path", None)
        elif operation_type in ["read_file", "delete_file"]:
            path = agent_request.query.params.get("file_path", None)
        ...
        try:
            if operation_type == "mount":
                ...
                file_path = agent_request.query.params.get("file_path", None)
                result = self.sto_read_file(file_path=file_path)

            elif operation_type == "list_dir":
                dir_path = agent_request.query.params.get("dir_path", None)
                recursive = agent_request.query.params.get("recursive", False)
                result = self.sto_list_directory(dir_path=dir_path, recursive=recursive)

            elif operation_type == "delete_file":
                file_path = agent_request.query.params.get("file_path", None)
                result = self.sto_delete_file(file_path=file_path)

            elif operation_type == "delete_dir":
                dir_path = agent_request.query.params.get("dir_path", None)
                recursive = agent_request.query.params.get("recursive", False)
                result = self.sto_delete_directory(dir_path=dir_path, recursive=recursive)
    ...
    def sto_read_file(self, file_path: str) -> dict:
        if not file_path:
    ...
    def sto_list_directory(self, dir_path: str, recursive: bool = False) -> dict:
        if not dir_path:
            return {"error": "dir_path is required"}
        try:
            if not os.path.exists(dir_path):
                return {"error": "Directory not found", "dir_path": dir_path}
            if not os.path.isdir(dir_path):
                return {"error": "Path is not a directory", "dir_path": dir_path}
            ...
            return {"error": f"Error listing directory: {str(e)}", "dir_path": dir_path}

    def sto_delete_file(self, file_path: str) -> dict:
        if not file_path:
            return {"error": "file_path is required"}
        lock = self.get_file_lock(file_path)
        try:
            ...
            return {"error": f"Error deleting file: {str(e)}", "file_path": file_path}

    def sto_delete_directory(self, dir_path: str, recursive: bool = False) -> dict:
        if not dir_path:
            return {"error": "dir_path is required"}
        try:
            if not os.path.exists(dir_path):
            ...
            if recursive:
                ...
                os.rmdir(dir_path)
            return {"deleted": True, "dir_path": dir_path, "recursive": bool(recursive)}
        except Exception as e:
            return {"error": f"Error deleting directory: {str(e)}", "dir_path": dir_path}
