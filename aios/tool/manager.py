...
class ToolManager:
    ...
    def _start_mcp_server(self):
        ...
        if os.getenv("NEWS_MCP_MANAGED_EXTERNALLY", "").lower() in {"1", "true", "yes", "on"}:
            print("MCP Server is managed by an external service. Skipping internal launch.")
            return
    ...
    def load_tool_instance(self, tool_org_and_name):
        local_candidates = []
        if "/" in tool_org_and_name:
            local_candidates.append(tool_org_and_name.split("/")[-1])
        else:
            local_candidates.append(tool_org_and_name)

        try:
            return AutoTool.from_preloaded(tool_org_and_name)
        except Exception as remote_exc:
            for candidate in local_candidates:
                try:
                    return AutoTool.from_preloaded(candidate, local=True)
                except Exception:
                    continue
            raise remote_exc
