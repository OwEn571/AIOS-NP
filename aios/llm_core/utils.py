...
def decode_litellm_tool_calls(response):
    ...
    # Handle string responses - they cannot contain tool calls
    if isinstance(response, str):
        logger.warning("Received string response when expecting tool calls. Returning empty list.")
        return decoded_tool_calls
    
    # Compatibility: some backends may already return a python list/dict of tool calls
    # Normalize to an object with choices[0].message.* shape when possible
    try:
        if isinstance(response, list):
            # Assume it's already a list of tool-call-like dicts/objects
            tool_calls = response
            # Convert into expected dict format
            for tool_call in tool_calls:
                # Support both dict and object with attributes
                if isinstance(tool_call, dict):
                    name = tool_call.get("name") or (tool_call.get("function", {}) or {}).get("name")
                    args = tool_call.get("arguments") or tool_call.get("args") or (tool_call.get("function", {}) or {}).get("arguments")
                else:
                    # Fallback attribute access
                    name = getattr(getattr(tool_call, "function", None), "name", None) or getattr(tool_call, "name", None)
                    args = getattr(getattr(tool_call, "function", None), "arguments", None) or getattr(tool_call, "arguments", None) or getattr(tool_call, "args", None)
        ...
        if isinstance(response, dict):
            # Possibly already a structured dict with tool_calls
            possible = response.get("tool_calls") or response.get("choices")
            if isinstance(possible, list):
                return decode_litellm_tool_calls(possible)
    except Exception:
        # Fall through to legacy path
        pass

    # Check if response has the expected structure before accessing choices
    if not hasattr(response, 'choices') or not response.choices:
        logger.warning("Response does not have expected 'choices' structure. Returning empty list.")
        return decoded_tool_calls
