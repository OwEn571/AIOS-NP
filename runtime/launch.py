...
def initialize_storage_manager(storage_config: dict) -> Any:
    """Initialize storage manager with configuration."""
    try:
        root_dir = storage_config.get("root_dir", str(ROOT_DIR))
        if not os.path.isabs(root_dir):
            root_dir = str((PROJECT_ROOT / root_dir).resolve())
...
def initialize_components_safe() -> dict:
    global startup_state

    try:
        active = initialize_components()
        startup_state["initialization_error"] = None
        startup_state["initialized_at"] = datetime.now().isoformat()
        return active
    except Exception as e:
        startup_state["initialization_error"] = str(e)
        startup_state["initialized_at"] = datetime.now().isoformat()
        print(f"⚠️ Kernel started in degraded mode: {e}")
        return {
            "llms": None,
            "storage": None,
            "memory": None,
...
# Initialize components when starting up
active_components = initialize_components_safe()
...
async def get_server_status():
    ...
    if not inactive_components:
        return {
            "status": "ok",
            "message": "All core components are active.",
            "initialized_at": startup_state["initialized_at"],
        }
    ...
    return {
        ...
        "message": f"Server is running, but some components are inactive: {', '.join(inactive_components)}",
        "inactive_components": inactive_components,
        "initialization_error": startup_state["initialization_error"],
        "initialized_at": startup_state["initialized_at"],
    }
...
async def refresh_configuration():
    ...
    try:
        ...
        restart_kernel()
        logger.info("Kernel restarted")
        if startup_state["initialization_error"]:
            return {
                "status": "warning",
                "message": "Configuration refreshed, but some components failed to initialize",
                "initialization_error": startup_state["initialization_error"],
            }
        return {
            "status": "success", 
...
async def get_status():
    ...
    component_status["initialization_error"] = startup_state["initialization_error"]
...
async def handle_query(request: QueryRequest):
    try:
        if request.query_type == "llm":
            ...
            query = rebuild_llm_query(request.query_data, query_required_llms)
            result_dict = await asyncio.to_thread(
                execute_request, # The method to call
                request.agent_name,               # First arg to execute_request
                query                # Second arg to execute_request
...
if __name__ == "__main__":
    ...
    server_config = config.get_server_config()
    host = server_config.get("host", "0.0.0.0")
    port = server_config.get("port", 5711)
